...
OUTPUT_HEADERS = OrderedDict([
    ...
    ('pes1', 'voted'),
    ('pes2', 'registered'),
    ('pes3', 'why_not_registered'),
    ('pes4', 'why_not_vote'),
    ('pes5', 'voted_how'),
    ('pes6', 'voted_on_election_day'),
    ('pes7', 'registered_how'),
    ('pes8', 'lived_at_current_address')
])
...
with open('data.csv') as f:
    ...
    for row in reader:
        ...
        for column, mapping in FIELD_MAPPINGS.items():
            ...
            output_row[OUTPUT_HEADERS[column]] = value

        output.append(output_row)

with open('simple.csv', 'w') as f:
    writer = csv.DictWriter(f, fieldnames=OUTPUT_HEADERS.values())
